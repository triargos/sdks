...
const handlers = [
    http.get('https://staging.dashboard.triargos.de/api/organizations', ({request}) => {
        const key = request.headers.get("X-Api-Key")
        if (!key) {
            return HttpResponse.json({message: "Authentication missing"}, {status: 401})
        }
        if (key === "forbidden") {
    ...
    http.get('https://staging.dashboard.triargos.de/api/organizations/invalid', ({request}) => {
        return HttpResponse.json({message: "Not found"}, {status: 404})
    }),
    http.get('https://staging.dashboard.triargos.de/api/organizations/invalid_schema', ({request}) => {
...
describe("handleExceptions", () => {
    ...
    it('should throw an resourceForbiddenException ', async () => {
        const dashboard = new Dashboard({apiKey: "forbidden", environment: "staging"})
        try {
            await dashboard.organizations.findAll()
        } catch (e) {
            expect(dashboard.get).toHaveBeenCalled
